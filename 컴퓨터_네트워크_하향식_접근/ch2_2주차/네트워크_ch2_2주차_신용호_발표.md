...
$^{*}$ 주로 유닉스 시스템 상에서 **BIND(Berkeley Internet Name Domain)** 소프트웨어로 운영

```
[참고] DNS가 UDP를 사용하는 이유

신뢰성보다 속도가 더 중요. (3-way-handshaking 불필요)
DNS 패킷의 크기가 작음. (UDP는 512b 이하 패킷만 전송 가능)
연결 상태 유지 불필요. (1회성 응답)
```
...
###### 사용자의 호스트에서 수행하는 브라우저가 URL을 검색했을 때 발생하는 일
  ...
  4. **DNS 서버**는 <u>호스트 이름에 대한 IP 주소</u>를 **DNS 클라이언트**로 전송
...
- **Root DNS Server**
    ...
    - *TLD 서버의 IP 주소* 제공
    - *인터넷 할당 번호 관리기관 ICANN(Internet Corporation for Assigned Names and Numbers)* 에 의해 조정

-  **TLD(Top-Level Domain) DNS Server**
    - `com`, `org`, `net` 같은 상위 레벨 도메인과 `kr`, `uk` 같은 모든 국가의 상위 레벨 도메인에 대한 TLD 서버가 있다.
    - *Authoritative(책임) DNS Server IP 주소* 제공
...
- `cse.nyu.edu`가 `gaia.cs.umass.edu`의 IP 주소를 알고 싶을 때
...
-  일반적으로 <u>TLD 서버는 위의 예시와 같이 책임 DNS 서버를 알지 않고, 책임 DNS 서버를 아는 중간 DNS 서버를 알고 있다.</u>

-  재귀적 질의 & 반복적 질의

- 위 예는 `재귀적 질의`와 `반복적 질의`를 사용한다.
- 아래 예는 모든 질의에서 `재귀적인 DNS 질의 사슬` 사용
...
- ***일반 질의는 전형적으로 반복적 질의***
  왜냐하면, `재귀적 질의`에서는 ***높은 계층 DNS 서버의 책임이 많아짐***
...
- **특정 hostname에 대한 책임 DNS 서버**는 **hostname에 대한 Type A 레코드**를 포함.
- **특정 hostname에 대한 책임 서버**가 아닌 경우 그 서버는 **hostname을 알고 있는 DNS 서버 IP 주소를 제공하는 Type A 레코드**를 포함.
...
- **Payload**
    ...
    - `Answers`
	  응답 메세지에서 질의된 이름에 대한 자원 레코드 포함.
	  여러 개의 자원 레코드 포함 가능 (하나의 호스트 이름을 가진 여러 개의 IP 존재 가능) (중복 웹 서버)
    - `Authority`
      다른 책임 서버의 레코드 포함
...
- 도메인 네임 등록시 `DNS 등록기관`에 **주책임 서버, 부책임 서버 각각의 이름 및 IP 주소** 제출 필요
- `DNS 등록기관`은 `Type NS`, `Type A` 레코드들을 **TLD 서버**(`com` 서버)에  등록**

- `주책임 서버`는  다음 **두 개의 자원 레코드**를 DNS 서버에 삽입하고, `Type A` 레코드와 메일 서버에 대한 `Type MX` 자원 레코드들의 **책임 DNS 서버 등록 확인**
...
- 인터넷 코어는 풍부한 대역폭을 가짐
- 다른 네트워크 애플리케이션은 참여 X. 즉, 업로드 및 다운로드 접속 대역폭 모두 사용
...
- **각 피어가 비트를 수신하자마자 그 비트를 재분배 할 수 있다고 가정**할 경우 (실제로는 chunk가 재분배됨) ${D_{P2P}}$ 는 다음과 같다.
...
- **토렌트(torrent)**
  특정 파일의 분배에 참여하는 모든 피어의 모임. 토렌트에 참여하는 피어들은 서로에게서 **같은 크기(일반적으로 256KB)의 청크(chunk)** 를 다운로드함. 처음 가입한 피어에는 청크가 없지만, 시간에 따라 점점 청크를 쌓게됨. **각 피어는 청크를 다운로드함과 동시에 다른 피어들에게 청크를 업로드할 수 있음.** 일단 한 피어가 전체 파일을 얻으면 토렌트를 떠나거나, 토렌트에 남아서 다른 피어들로 청크를 계속해서 업로드할 수 있다.
